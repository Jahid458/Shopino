...
function PriorityBadge({ priority }) {
  const cls = priority === "High" ? "bg-red-100 text-red-500"
    : priority === "Medium" ? "bg-sky-100 text-sky-600"
    : "bg-slate-100 text-slate-500";
  return <span className={`text-xs px-3 py-1.5 rounded-full font-semibold ${cls}`}>{priority || "Normal"} Priority</span>;
}
    ...
    <Link href={`/items/${item._id}`}
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-1">
          ...
          <span className="text-sm font-bold text-sky-600 shrink-0">${Number(item.price).toFixed(2)}</span>
        </div>
        <p className="text-xs text-slate-400 line-clamp-2 mb-3">{item.fullDescription}</p>
        <div className="flex items-center justify-between">
          <span className="text-xs px-2.5 py-1 rounded-full bg-sky-100 text-sky-600 font-medium">{item.category}</span>
          <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${
            item.priority === "High" ? "bg-red-100 text-red-500"
            : item.priority === "Medium" ? "bg-sky-100 text-sky-600"
            : "bg-slate-100 text-slate-500"}`}>
            {item.priority}
          </span>
        </div>
...
function ProductDetailContent() {
  ...
  async function fetchProduct() {
    ...
    try {
      ...
      if (!res.ok || !data.success) { setError(data.message || "Product not found."); return; }
  ...
  async function fetchRelated(category, currentId) {
    try {
      ...
      const data = await res.json();
      if (data.success) setRelated(data.products.filter((p) => p._id !== currentId).slice(0, 3));
    } catch {}
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-sky-100 py-10 px-4 sm:px-6">
      ...
      {showDeleteModal && (
        <DeleteModal onConfirm={handleDelete} onCancel={() => setShowDeleteModal(false)} deleting={deleting} />
      )}
            ...
            <div className="relative bg-sky-50">
              {product.imageUrl ? (
                <img src={product.imageUrl} alt={product.title}
                  className="w-full h-64 sm:h-80 lg:h-full max-h-[520px] object-cover"
                  onError={(e) => { e.target.src = "https://placehold.co/800x600/e0f2fe/0ea5e9?text=No+Image"; }} />
              ) : (
                ...
                {product.date && (
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-slate-700">{product.date}</p>
                    </div>
                  </div>
                )}
                {product.createdBy && (
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-slate-700 truncate max-w-[8rem]">{product.createdBy}</p>
                    </div>
                  </div>
                )}
                <div className="flex items-start gap-2">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={() => setWished(!wished)}
                  className={`flex items-center justify-center h-12 w-12 rounded-xl border transition-colors ${
                    wished ? "bg-red-50 border-red-200 text-red-500" : "border-slate-200 text-slate-400 hover:border-sky-300 hover:text-sky-500"
                  }`}>
        ...
        {related.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {related.map((item) => <RelatedCard key={item._id} item={item} />)}
            </div>
          </section>
        )}
